...
/*
    ...
    private final static String DELETE_BOOK_SQL="delete from lend_list where sernum = ?  ";
    private final static String DELETE_BOOK_SQL_ID="delete from book_info where book_id = ?  ";
    ...
    public int deleteBook2(long bookid){
        ...
        return jdbcTemplate.update(DELETE_BOOK_SQL_ID,bookid);
    }
...
*/
    ...
    @Select("SELECT count(*) FROM book_info WHERE bookId  like CONCAT('%',#{searchWord},'%') or name like #{searchWord} ")
    public int matchBook(String searchWord);
    @Select("SELECT * FROM book_info WHERE bookId like CONCAT('%',#{sw},'%')  or name like #{sw} ")
    public ArrayList<Book> queryBook(String sw);
    @Select("SELECT * FROM book_info ")
    public ArrayList<Book> getAllBooks();
    @Delete("delete from lend_list where sernum =#{sernum} ")
    public int deleteBook(long sernum);
    @Delete("delete from book_info where bookId = #{bookId} ")
    public int deleteBook2(long bookid);
    ...
    public int addBook(Book book);
    @Select("SELECT * FROM book_info where bookId = #{bookId} ")
    public Book getBook(Long bookId);
    @Update("update book_info set name= #{name} ,author=#{author} ,publish=#{publish} ,ISBN= #{isbn} ,introduction=#{introduction} ,language= #{language} ,price= #{price},pubdate= #{pubdate} ,classId= #{classId} ,pressmark= #{pressmark},state= #{state}  where bookId= #{bookId}")
    public int editBook(Book book);
